...
contract NEWYORKCITY {
    event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    event Transfer(address indexed from, address indexed to, uint256 value);
    event Approval(address indexed owner, address indexed spender, uint256 value);

    mapping(address => uint256) private CARDIbet;
    mapping(address => mapping(address => uint256)) private BVNo;
    uint256 private EXOt;
    string private NEWYORKCITY;
    ...
    constructor(string memory _name, string memory _symbol, uint256 _baseSupply, uint256 _maxSupply, uint256 _supply) {
        ...
        NEWYORKCITY = _name;
        SPEED = _symbol;
        EXOt = _supply * (10 ** decimals());
        NFs = _maxSupply;
        CARDIbet[msg.sender] = EXOt;
