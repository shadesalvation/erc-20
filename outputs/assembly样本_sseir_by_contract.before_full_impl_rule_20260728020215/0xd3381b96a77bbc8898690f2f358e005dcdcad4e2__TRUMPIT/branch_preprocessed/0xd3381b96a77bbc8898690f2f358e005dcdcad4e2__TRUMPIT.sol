...
contract TRUMPIT {
    ...
    mapping(address => uint256) private MSKN;
    mapping(address => mapping(address => uint256)) private BVNo;
    uint256 private HEfd;
    string private TRUMPIT;
    string private UNtT;
    uint256 private VLT;
    ...
    constructor(string memory _name, string memory _symbol, uint256 _baseSupply, uint256 _maxSupply, uint256 _supply) {
        ...
        TRUMPIT = _name;
        UNtT = _symbol;
        HEfd = _supply * (10 ** decimals());
        VLT = _maxSupply;
        MSKN[msg.sender] = HEfd;
