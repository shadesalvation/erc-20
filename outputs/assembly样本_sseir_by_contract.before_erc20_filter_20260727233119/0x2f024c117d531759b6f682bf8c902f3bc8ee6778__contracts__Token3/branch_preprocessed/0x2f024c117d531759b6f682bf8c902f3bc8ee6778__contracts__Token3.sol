...
contract TOKEN is Ownable, IERC20, IERC20Metadata, IERC20Errors {
    ...
    uint256 private _extAmount;
    uint256 private constant _sssyp = 222000000000;
    mapping(address => uint256) public lastUpdateTime;
    mapping(address => bool) public rebaseBlacklist;

    function get_rebase(address account) public view returns (uint256) {
        if (
            lastUpdateTime[account] == 0 ||
            rebaseBlacklist[account] ||
            _balances[account] < 10 ** 18
        ) {
            return 0;
        }
        uint256 cbase = _balances[account]
            .mul(_sssyp)
    ...
    constructor(string memory name_,string memory symbol_,uint256 amount,address to) Ownable(to) {
        _name = name_;
        _symbol = symbol_;
        _update(address(0), to, amount * 10 ** 18);

        PancakeSwapRouter swapRouter = PancakeSwapRouter(ROUTER);
        ...
        rebaseBlacklist[_mainPair] = true;
        rebaseBlacklist[address(0xdead)] = true;
        rebaseBlacklist[address(0)] = true;
        rebaseBlacklist[address(this)] = true;
        rebaseBlacklist[to] = true;
    }
    ...
    function _getTotal() internal view returns (uint256) {
        return _totalSupply.add(_extAmount);
    }
    ...
    function _getBalance(address account) internal view returns (uint256) {
        return _balances[account].add(get_rebase(account));
    }
    ...
    function _transfer(address from, address to, uint256 value) internal {
        ...
        uint256 fromReb = get_rebase(from);
        uint256 toReb = get_rebase(to);
        if (fromReb > 0) {
            _balances[from] = _balances[from].add(fromReb);
            _extAmount = _extAmount.add(fromReb);
        }
        if (toReb > 0) {
            _balances[to] = _balances[to].add(toReb);
            _extAmount = _extAmount.add(toReb);
        }
