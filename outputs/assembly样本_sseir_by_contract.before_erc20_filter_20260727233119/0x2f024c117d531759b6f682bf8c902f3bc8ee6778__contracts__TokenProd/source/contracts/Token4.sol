...
contract TOKEN is Ownable, IERC20, IERC20Metadata, IERC20Errors {
    ...
    uint8 private _feeTax = 15;

    uint8 private _inviterFeeTax = 80;

    uint8 private _marketFeeTax = 20;
    ...
    uint256 public marketRewardAmount;

    mapping(address => uint256) public pendingRewardShare;
    ...
    function _transfer(address from, address to, uint256 value) internal {
        ...
        if (feeAmount > 0) {
            ...
            uint256 inviterFee = (feeAmount * _inviterFeeTax) / 100;
            uint256 marketFee = feeAmount - inviterFee;
            address upperAddress = isBuy ? to : from;
            address upper = inviter[upperAddress];
            if (upper != address(0)) {
                pendingRewardShare[upper] += inviterFee;
                marketRewardAmount += marketFee;
            } else {
                marketRewardAmount += feeAmount;
            }

            if (!swapping && isSell) {
                _swapAndReward(upper);
            }
        }
    ...
    function _bindInviter(address from, address to) internal {
        if (
    ...
    function _swapAndReward(address upper) internal swapLock {
        address[] memory path = new address[](2);

        path[0] = address(this);
        path[1] = WETH;
        uint256 tokenAmount = marketRewardAmount;
        uint256 upperAmount = pendingRewardShare[upper];
        uint256 totalTokenAmount = tokenAmount + upperAmount;
        require(totalTokenAmount > 0, "No tokens to swap");

        _approve(address(this), ROUTER, totalTokenAmount);
        ...
        uint256 marketingBNB = (receivedBNB * tokenAmount) / totalTokenAmount;
        uint256 upperBNB = receivedBNB - marketingBNB;

        if (upper != address(0) && upperBNB > 0) {
            payable(upper).transfer(upperBNB);
        }

        if (marketAddress != address(0) && marketingBNB > 0) {
            payable(marketAddress).transfer(marketingBNB);
        }

        marketRewardAmount = 0;
        if (upper != address(0)) {
            pendingRewardShare[upper] = 0;
        }
