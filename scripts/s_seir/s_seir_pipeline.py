...
def build_sseir(source_path:Path, solc_bin:str|None=None, slither_bin:str|None=None, workdir:Path|None=None):
    ...
    for unit in SourceStatementCollector(source_path,ast).collect():
        ...
        out.append(FunctionSSEIR(unit.function_id,unit.contract,unit.function,unit.signature,unit.source_statements,control,roles,effects,overlays,policies,security_facts,json_ready(facts)))
    return out
