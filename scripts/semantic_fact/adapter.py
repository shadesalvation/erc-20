...
class SSeirFactAdapter:
    ...
    def function_facts(self, fn: dict[str, Any]) -> list[SemanticFact]:
        ...
        overlays = [item for item in fn.get("semantic_overlays") or [] if isinstance(item, dict)]
        encoded_call_effects = {
            tuple(sorted(str(effect) for effect in overlay.get("effects") or []))
            for overlay in overlays
            if overlay.get("kind") == "AbiEncodedLowLevelCall"
        }
        for overlay in overlays:
            if not isinstance(overlay, dict):
                continue
            effect_key = tuple(sorted(str(effect) for effect in overlay.get("effects") or []))
            if (
                overlay.get("kind") in {"LowLevelCall", "StaticCallOverlay", "DelegateCallOverlay"}
                and effect_key in encoded_call_effects
            ):
                continue
    ...
    def overlay_facts(
        ...
        if kind == "PathConditionedRawReturnData":
            return [
                self.return_candidate_fact(fn, overlay, candidate, stmt_lang)
                for candidate in attrs.get("candidates") or []
                if isinstance(candidate, dict)
            ]
    ...
    def plain_overlay_fact(
        ...
        if kind in {"CustomErrorRevert", "RawRevertBytes", "RevertOverlay"}:
            payload = clean_dict({
                "kind": "RevertPayload",
                "pointer": attrs.get("payload_ptr"),
                "size": attrs.get("payload_size"),
                "values": attrs.get("args") or attrs.get("values"),
                "source_object": attrs.get("source_object"),
                "encoding": attrs.get("encoding_hint"),
                "complete": attrs.get("payload_memory_complete"),
                "unresolved_reason": attrs.get("unresolved_reason"),
            })
        ...
        if kind in {"ExternalCall", "LowLevelCall", "StaticCallOverlay", "DelegateCallOverlay", "AbiEncodedLowLevelCall"}:
        ...
        if kind == "ReturnValue":
            values = attrs.get("values") or clean_list([
                attrs.get("value") or attrs.get("return_value") or attrs.get("expression")
            ])
        ...
        if kind == "RawReturnData":
            values = list(attrs.get("values") or [])
            complete = bool(attrs.get("payload_memory_complete"))
            payload = clean_dict({
                "kind": "ReturnPayload",
                "pointer": attrs.get("payload_ptr_normalized") or attrs.get("payload_ptr"),
                "size": attrs.get("payload_size_normalized") or attrs.get("payload_size"),
                "values": values,
                "encoding": attrs.get("encoding_hint") or "raw_memory",
                "complete": complete,
                "unresolved_reason": None if complete else "memory_source_unresolved",
            })
            reads = values or clean_list([attrs.get("payload_ptr"), attrs.get("payload_size")])
            return self.fact(fn, overlay, stmt_lang, "Return", rvalue=values or None, reads=reads, semantic={
                "operation": "raw_return",
                "values": values,
                "return_payload": payload,
                "exact_solidity_equivalent": attrs.get("solidity_equivalent"),
            })
    ...
    def revert_candidate_fact(self, fn: dict[str, Any], overlay: dict[str, Any], candidate: dict[str, Any], stmt_lang: dict[str, str]) -> SemanticFact:
        attrs = overlay.get("attrs") or {}
        values = list(candidate.get("values") or candidate.get("args") or [])
        resolved = self.candidate_is_resolved(candidate)
        return self.fact(fn, overlay, stmt_lang, "Revert", condition=candidate.get("condition"), reads=flat_list(candidate.get("args")), semantic={
            ...
            "revert_payload": clean_dict({
                "kind": "RevertPayload",
                "pointer": attrs.get("payload_ptr"),
                "size": attrs.get("payload_size"),
                "selector": candidate.get("selector"),
                "values": values,
                "encoding": candidate.get("encoding_hint") or "raw_memory",
                "complete": resolved,
                "unresolved_reason": candidate.get("unresolved_reason") or candidate.get("reason") if not resolved else None,
            }),
            "candidate_status": candidate.get("status"),
            "unresolved_reason": candidate.get("unresolved_reason") or candidate.get("reason"),
        }, extra_evidence={"candidate": clean_dict(candidate)})

    def return_candidate_fact(self, fn: dict[str, Any], overlay: dict[str, Any], candidate: dict[str, Any], stmt_lang: dict[str, str]) -> SemanticFact:
        attrs = overlay.get("attrs") or {}
        ...
        return self.fact(
            ...
            "Return",
            condition=candidate.get("condition"),
            rvalue=values or None,
            reads=values or clean_list([attrs.get("payload_ptr"), attrs.get("payload_size")]),
            semantic={
                "operation": "raw_return",
                "values": values,
                "return_payload": clean_dict({
                    "kind": "ReturnPayload",
                    "pointer": attrs.get("payload_ptr"),
                    "size": attrs.get("payload_size"),
                    "values": values,
    ...
    def call_candidate_fact(self, fn: dict[str, Any], overlay: dict[str, Any], candidate: dict[str, Any], stmt_lang: dict[str, str]) -> SemanticFact:
        ...
        arguments = candidate.get("arguments") or candidate.get("args") or candidate.get("values") or []
        resolved = self.candidate_is_resolved(candidate)
        success_result = candidate.get("success_result") or candidate.get("result") or attrs.get("success_result") or attrs.get("result_name") or attrs.get("result")
        return self.fact(fn, overlay, stmt_lang, base_kind, condition=candidate.get("condition"), lvalue=success_result, reads=flat_list(candidate.get("arguments") or candidate.get("args")), writes=[success_result], semantic={
            "target": candidate.get("target_solidity") or candidate.get("target") or attrs.get("target"),
            ...
            "call_data": clean_dict({
                "kind": "CallData",
                "pointer": attrs.get("input_ptr"),
                "size": attrs.get("input_size"),
                "selector": candidate.get("selector"),
                "values": arguments,
                "encoding": "abi" if candidate.get("selector") else "raw_memory",
                "complete": resolved,
                "unresolved_reason": candidate.get("reason") if not resolved else None,
            }),
            "candidate_status": candidate.get("status"),
            "unresolved_reason": candidate.get("unresolved_reason") or candidate.get("reason"),
        }, extra_evidence={"candidate": clean_dict(candidate)})
    ...
    @staticmethod
    def candidate_is_resolved(candidate: dict[str, Any]) -> bool:
        """Accept compact S-SEIR candidates where resolved status is omitted.

        Missing status alone is not enough: compact candidates must still
        contain a recovered semantic value or expression.
        """
        status = candidate.get("status")
        if status not in {None, "", "resolved"}:
            return False
        if candidate.get("unresolved_reason") or candidate.get("reason"):
            return False
        return status == "resolved" or bool(
            ...
            or candidate.get("solidity_like")
        )

    def call_fact(self, fn: dict[str, Any], overlay: dict[str, Any], stmt_lang: dict[str, str], fact_kind: str) -> SemanticFact:
        attrs = overlay.get("attrs") or {}
        # An explicitly empty decoded argument list is meaningful.  Falling
        # back to ``args`` would incorrectly expose the seven Yul call
        # operands as ABI arguments.
        arguments = attrs.get("arguments") if "arguments" in attrs else attrs.get("args")
        abi_calldata = attrs.get("abi_calldata") or {}
        decoded_input = attrs.get("decoded_input")
        payload_values = list(arguments or [])
        if not payload_values and decoded_input:
            payload_values = [decoded_input]
        encoding = "abi" if abi_calldata or attrs.get("selector") else "raw_memory"
        call_data = clean_dict({
            "kind": "CallData",
            "pointer": attrs.get("input_ptr"),
            "size": attrs.get("input_size"),
            "selector": attrs.get("selector") or abi_calldata.get("selector"),
            "signature": attrs.get("selector_signature") or abi_calldata.get("signature"),
            "values": payload_values,
            "encoding": encoding,
            "complete": bool(abi_calldata or decoded_input is not None),
            "unresolved_reason": None if (abi_calldata or decoded_input is not None) else attrs.get("unresolved_reason") or "memory_source_unresolved",
        })
        success_result = attrs.get("success_result") or attrs.get("result_name") or attrs.get("result")
        return self.fact(fn, overlay, stmt_lang, fact_kind, lvalue=success_result, reads=clean_list([
            attrs.get("target_solidity") or attrs.get("target"),
            attrs.get("value"),
            *flat_list(arguments),
        ]), writes=[success_result], semantic={
    ...
    def sink_effect_kinds(overlay_kind: str) -> set[str]:
        ...
        if overlay_kind in {"ReturnValue", "RawReturnData", "PathConditionedRawReturnData"}:
            return {"Return"}
        return set()
