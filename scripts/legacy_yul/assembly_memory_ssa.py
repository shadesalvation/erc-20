...
class MemoryDefinition:
    ...
    aliases: tuple[AddressAlias, ...] = field(default_factory=tuple, compare=False, hash=False)
...
def parse_int_literal_local(text: str | None) -> int | None:
    if text is None:
        return None
...
def alias_expression(base: str, offset: int) -> str:
    if offset == 0:
        return base
    return f"add({base}, {offset})"


def normalize_alias_key(base: str, offset: int) -> str:
    return normalize_expression(alias_expression(base, offset))
...
class MemorySSAAnalyzer:
    ...
    def linear_aliases(self, node: Json, state: PathState, seen: set[str] | None = None) -> list[AddressAlias]:
        """Resolve a memory address into linear aliases over known symbolic bases.

        This deliberately handles only linear add/sub with constant offsets. It
        keeps the original expression as an alias and follows value definitions
        only when they themselves are linear address expressions. Non-linear
        definitions stop at the variable name, so mload(0x40) remains a symbolic
        base such as `ptr` instead of becoming `mload(0x40)`.
        """
        seen = seen or set()
        original = yul_expression(node)
        node_type = node.get("nodeType") if isinstance(node, dict) else None
        ...
        if node_type == "YulIdentifier":
            ...
            aliases = [AddressAlias(normalize_alias_key(name, 0), name, 0, name, "identifier")]
            if name in seen:
                return aliases
            ...
            call_name, _arguments = direct_call(expr)
            # Only chase values that are address-linear. Calls like mload(0x40)
            # define a new symbolic base and should not be expanded further.
            if call_name in {"add", "sub"} or (isinstance(expr, dict) and expr.get("nodeType") == "YulIdentifier"):
                for alias in self.linear_aliases(expr, state, seen | {name}):
                    if alias.key not in {item.key for item in aliases}:
                        aliases.append(AddressAlias(alias.key, alias.base, alias.offset, alias.expression, f"value:{definition.version}"))
            return aliases

        call_name, arguments = direct_call(node if isinstance(node, dict) else None)
        if call_name in {"add", "sub"} and len(arguments) == 2:
            left, right = arguments
            right_const = parse_int_literal_local(yul_expression(right))
            left_const = parse_int_literal_local(yul_expression(left))
            if right_const is not None:
                delta = right_const if call_name == "add" else -right_const
                return self.shift_aliases(self.linear_aliases(left, state, seen), delta, original)
            if call_name == "add" and left_const is not None:
                return self.shift_aliases(self.linear_aliases(right, state, seen), left_const, original)

        return [AddressAlias(normalize_expression(original), original, 0, original, "symbolic")]

    @staticmethod
    def shift_aliases(aliases: list[AddressAlias], delta: int, original: str) -> list[AddressAlias]:
        shifted: list[AddressAlias] = []
        seen_keys: set[str] = set()
        for alias in aliases:
            offset = alias.offset + delta
            key = normalize_alias_key(alias.base, offset)
            if key in seen_keys:
                continue
            seen_keys.add(key)
            shifted.append(AddressAlias(key, alias.base, offset, alias_expression(alias.base, offset), alias.source))
        original_key = normalize_expression(original)
        if original_key not in seen_keys:
            shifted.append(AddressAlias(original_key, original, 0, original, "original"))
        return shifted

    def write_memory_definition(
        self,
        node_id: int,
        address_node: Json,
        value: str,
        ...
        aliases = tuple(self.linear_aliases(address_node, state))
        primary = aliases[0].key if aliases else address_key(address_node)
        definition = self.new_memory_definition(node_id, primary, value, kind, src, state.memory, aliases)
        for alias in aliases:
            state.memory[alias.key] = definition
        return definition
    ...
    def apply_local_function_summary(
        ...
        for pointer, value in summary.memory_writes:
            ...
            alias = AddressAlias(address, address, 0, address, "local_function")
    ...
    def transfer(self, node_id: int, incoming: PathState) -> PathState:
        ...
        if call_name in MEMORY_WRITES and len(arguments) >= 2:
            ...
            definition = self.write_memory_definition(
                node_id,
                arguments[0],
                value,
                call_name,
            ...
            aliases = ", ".join(alias.key for alias in definition.aliases) or definition.address
            observation.effect = f"{definition.version}: {call_name}({definition.address}, {value}) aliases=[{aliases}]"
            return state
        ...
        if call_name in MEMORY_COPIES and arguments:
            ...
            aliases = ", ".join(alias.key for alias in definition.aliases) or definition.address
            observation.effect = f"{definition.version}: {call_name} writes unknown range at {definition.address} aliases=[{aliases}]"
            return state
...
def format_memory_ssa(result: MemorySSAResult) -> str:
    ...
    for definition in result.memory_definitions.values():
        alias_text = ", ".join(f"{alias.expression}[{alias.source}]" for alias in definition.aliases) or definition.address
        lines.append(
            f"  {definition.version} = {definition.kind}({definition.address}, {definition.value}) "
            f"aliases=[{alias_text}] origin=N{definition.node_id} src={definition.origin_src}"
        )
