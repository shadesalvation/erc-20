...
def variable_info(node:Json,kind:str)->VariableInfo:
    t=node.get('typeDescriptions',{}).get('typeString')
    if not t:
        tn=node.get('typeName') or {}; t=tn.get('name') or tn.get('nodeType') or 'unknown'
    return VariableInfo(node.get('name',''),kind,str(t),node.get('storageLocation') or None,str(node.get('src','')))
...
class SourceStatementCollector:
    ...
    def collect_function(self,contract:str,fn:Json)->FunctionUnit:
        ids=IdAllocator(); params,rets,locals_=collect_variables(fn); name=function_name(fn); sig=f"{name}({', '.join(v.type_string for v in params)})"; fid=f"{contract}.{sig}"
        ...
        unit=FunctionUnit(fid,contract,name,sig,fn,params,rets,locals_)
